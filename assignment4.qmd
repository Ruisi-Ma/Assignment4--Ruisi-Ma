...
Github Link for Assignment4: <https://github.com/Ruisi-Ma/Assignment4--Ruisi-Ma.git>
...
```{sql connection = con}
SELECT primary_type, count(*) AS arrestcount
FROM crime
...
```{sql connection = con}
SELECT EXTRACT(HOUR FROM date), count(*) AS arrestnumber
FROM crime
WHERE year = 2016 AND arrest = TRUE
GROUP BY EXTRACT(HOUR FROM date)
ORDER BY arrestnumber DESC
```

The 19th hour(19:00-19:59) is associated with the most arrest during a day.
...
```{sql connection = con}
SELECT year, count(*) AS arrestcount
FROM crime
...
```{sql connection = con}
SELECT year, district, COUNT(*) AS arrestnumber
FROM crime
...
In 2015 and 2016, the 11th district have the highest numbers of arrests among all districts.
...
```{r}
crimetbl <- tbl(con, "crime")
str(crimetbl)
class(crimetbl)
```

Again, count the number of arrests grouped by `primary_type` of district 11 in year 2016, now using `dplyr` syntax.Count the number of arrests grouped by `primary_type` and `year`, still only for district 11. Arrange the result by `year`. Assign the results of the query above to a local R object.

```{r}
arrestcount2016 <- crimetbl %>%
  filter(year == 2016, district == 11, arrest == TRUE) %>%
  group_by(primary_type) %>%
  summarize(arrestcount = n(),.groups = 'drop') %>%
  arrange(desc(arrestcount)) %>%
  collect()
```

```{r}
arrest_year <- crimetbl %>%
  filter(district == 11, arrest == TRUE) %>%
  group_by(primary_type, year) %>%
  summarize(arrestnumber = n(), .groups = 'drop') %>%
  arrange(year) %>%
  collect()
```
...
```{r}
print(head(arrestcount2016, 10))
print(head(arrest_year, 10))
```
